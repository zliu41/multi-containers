...
fromRowMap :: Ord c => Map r (Map c a) -> Table r c a
fromRowMap m = Table (m', transpose m', size' m')
  where m' = nonEmpty m

-- | Build a table from a column map.
fromColumnMap :: Ord r => Map c (Map r a) -> Table r c a
...
-- | Return the list of all row keys of that have at least one value
-- in the table.
rowKeys :: Table r c a -> [r]
rowKeys (Table (rm, _, _)) = Map.keys rm

-- | Return the list of all column keys of that have at least one value
-- in the table.
...
transpose :: Ord c => Map r (Map c a) -> Map c (Map r a)
transpose = Map.foldrWithKey' f Map.empty
  where
    f r = Map.union . Map.map (Map.singleton r)
